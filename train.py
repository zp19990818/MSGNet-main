...
from datasets.dataloader_list.adaptor_dataset import DeepFish_Dataset
...
def init_dataloaders(args):
    ...
    for split in ['train', 'val']:
        ...
        dataset = DeepFish_Dataset(split=split, datasets=datasets, augment=True, transform=image_transforms, target_transform=target_transforms)
...
def trainIters(args):
    ...
    model_dir = os.path.join('ckpt/official_DeepFish/MSGNet_100epoch_emptyflow', args.model_name)
    make_dir(model_dir)
    now = datetime.datetime.now()  # 获得当前时间
    timestr = now.strftime("%Y_%m_%d_%H_%M_%S")
    log_path = os.path.join(model_dir, timestr + '.txt')
...
if __name__ == '__main__':
    ...
    args.model_name = 'MSGNet'
    args.datasets = ["DeepFish_Seagrass"]
    args.batch_size = 4
    args.max_epoch = 100
    args.lr = 1e-3
    args.snapshot = ''
